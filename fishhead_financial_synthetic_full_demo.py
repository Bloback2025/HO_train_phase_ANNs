# fishhead_financial_synthetic_full_demo.py
# -------------------------------------------------------------------
# Fishhead ANN demo on synthetic *financial-like* data.
#
# Features:
#   - Synthetic generator with drift/volatility regimes, jumps, mean reversion
#   - Multi-head ANN (residual, quantiles, event probability, gate)
#   - Training loop with loss logging
#   - Three plots saved to PNGs:
#       1. Quantile bands vs true ΔClose
#       2. Event probability calibration curve
#       3. Gate/abstention behaviour (low-confidence sit-outs)
#
# Purpose:
...
# BROKEN:     with torch.no_grad():
        ...
        gate_scores = out["gate"].numpy().flatten()
    ...
    # --- Plot 3: Gate/abstention behaviour ---
    plt.figure(figsize=(10,4))
    plt.plot(gate_scores, label="Gate score (confidence)")
    plt.axhline(0.5, color="red", linestyle="--", label="Threshold 0.5")
    plt.title("Fishhead Gate/Abstention Behaviour")
    plt.xlabel("Time step")
    plt.ylabel("Gate score")
    plt.legend()
    plt.savefig("fishhead_financial_gate.png")
    plt.close()
