...
# BROKEN: def make_dataset_from_close(close: np.ndarray, window=WINDOW, event_thresh=EVENT_REL_THRESHOLD):
    """
    Features: normalized window of closes (demeaned), shape=(window,)
    Target: ΔClose = Close[t+1] - Close[t]  (one-day ahead)
    Event: 1 if |(Close[t+1]-Close[t])/Close[t]| >= event_thresh else 0
    Naive baseline for ΔClose: 0.0
    """
    X, y, ev, pc_forecast = [], [], [], []
# BROKEN:     for i in range(window, len(close)-1):
        ...
        pc_forecast.append(np.float32(0.0))  # Naïve ΔClose is zero
...
# BROKEN:     def __init__(self, input_dim=WINDOW, hidden=96, dropout=0.1):
        ...
        self.resid = nn.Linear(hidden, 1)   # ΔClose regression (q50 proxy)
...
# BROKEN: def mape_safe(y_true, y_pred, eps=1e-6):
    # Handles ΔClose near zero without exploding
...
# BROKEN:     with torch.no_grad():
    ...
    # Choose q50 as point forecast for ANN
    yhat_v = q_v[:,1]
    yhat_s = q_s[:,1]
    ...
    # Event truth for Brier (use prev_close alignment)
    ...
    abstain_v = float(np.mean(g_v < 0.3))  # configurable threshold
    ...
    metrics_csv = os.path.join(OUTDIR, f"HO_Fishhead_metrics_v5_1_{ts}.csv")
    metrics_df.to_csv(metrics_csv, index=False)

    model_path = os.path.join(OUTDIR, f"Fishhead_HO_model_v5_1_{ts}.pt")
    torch.save({"state_dict": model.state_dict(),
                "config": {"input_dim": X.shape[1], "hidden": 96, "dropout": 0.1,
    ...
    runlog_path = os.path.join(OUTDIR, f"RUNLOG_Fishhead_HO_v5_1_{ts}.json")
# BROKEN:     with open(runlog_path, "w") as f:
    ...
    plt.tight_layout(); plt.savefig(os.path.join(OUTDIR, f"val_true_q50_naive_v5_1_{ts}.png")); plt.close()
    ...
    plt.tight_layout(); plt.savefig(os.path.join(OUTDIR, f"val_quantile_coverage_v5_1_{ts}.png")); plt.close()
    ...
    plt.legend(); plt.tight_layout(); plt.savefig(os.path.join(OUTDIR, f"val_event_calibration_v5_1_{ts}.png")); plt.close()
    ...
    plt.tight_layout(); plt.savefig(os.path.join(OUTDIR, f"val_gate_v5_1_{ts}.png")); plt.close()

    # Console printout (closure marker)
    print("\n=== Fishhead HO Audit v5.1 Metrics ===")
