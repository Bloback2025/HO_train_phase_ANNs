...
df=pd.read_csv(r".\hoxnc_testing_with_target_lagged_trimmed.csv")
X=df[m["features"]].astype(float).values
y=df[m["target"]].astype(float).values
...
# BROKEN: if mask.sum()>2:
    r=np.corrcoef(pred[mask],y[mask])[0,1]; a,b=np.polyfit(pred[mask],y[mask],1)
    print("[FIT] corr=",float(r)," slope=",float(a)," intercept=",float(b))
# BROKEN: else:
    print("[FIT] insufficient valid pairs")
print("[X] nan_total=",int(np.isnan(X).sum()))
