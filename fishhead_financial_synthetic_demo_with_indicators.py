# fishhead_financial_synthetic_demo_with_indicators.py
# -------------------------------------------------------------------
# Fishhead ANN demo on synthetic *financial-like* data.
#
# Extended version: in addition to training and plotting, this script
# now computes diagnostic indicators (MAE, RMSE, R², MAPE) for the ANN
# compared against a persistence baseline. Results are printed and
# written to JSON for reproducibility.
# -------------------------------------------------------------------
...
# BROKEN:     with torch.no_grad():
        out = model(X_tensor)
        y_pred = out["residual"].numpy().flatten()
        q_preds = out["quantiles"].numpy()
        event_probs = out["event_prob"].numpy().flatten()

    # --- Persistence baseline (ΔClose_t = 0, i.e. no change forecast) ---
    y_persist = np.zeros_like(y.flatten())

        # --- Compute diagnostics ---
    mae_ann = float(mean_absolute_error(y, y_pred))
    mae_persist = float(mean_absolute_error(y, y_persist))

    rmse_ann = float(mean_squared_error(y, y_pred) ** 0.5)
    rmse_persist = float(mean_squared_error(y, y_persist) ** 0.5)

    r2_ann = float(r2_score(y, y_pred))
    r2_persist = float(r2_score(y, y_persist))

    mape_ann = float(np.mean(np.abs((y.flatten() - y_pred) / y.flatten())) * 100)
    mape_persist = float(np.mean(np.abs((y.flatten() - y_persist) / y.flatten())) * 100)
    ...
    results = {
        ...
        "mape_ann": mape_ann,
        "mape_persistence": mape_persist,
    }

    print("Edge diagnostics:", json.dumps(results, indent=2))

# BROKEN:     with open("fishhead_synthetic_manifest.json", "w") as f:
