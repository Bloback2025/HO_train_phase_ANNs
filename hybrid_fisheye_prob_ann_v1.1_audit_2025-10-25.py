...
# ---------------------------
# BROKEN: def load_df(path, date_fmt="%d-%b-%y"):
    df = pd.read_csv(path)
...
# Model components
# ---------------------------
# BROKEN: def fisheye_block(x, embed_dim=EMBED_DIM, attn_dim=ATTN_DIM, dropout=DROPOUT, l2=L2W):
    # x: [B, T, F]
...
# BROKEN: def probabilistic_head(x, l2=L2W):
...
# BROKEN: def gaussian_nll(y_true, mu, log_var, eps=1e-6):
    var = tf.exp(log_var) + eps
    return 0.5 * (tf.math.log(var) + tf.square(y_true - mu) / var)

# BROKEN: def build_model(context_len=CONTEXT_LEN, feat_dim=FEATURES_PER_T):
