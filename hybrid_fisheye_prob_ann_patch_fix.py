# Name: hybrid_fisheye_prob_ann_patch_fix.py
# Date: 2025-10-24
# Purpose: Non-destructive diagnostics and targeted model-fix for hybrid_fisheye_prob_ann POC.
# Notes:
# - Does NOT change input files or targets.
# - Adds assertive, auditable checks for scaling and sequence->target alignment.
# - Adds combined loss (NLL + alpha * MAE(mu)), clipped log_sigma, optional init of mu bias
#   to the training-set persistence residual (keeps inputs identical; only weight init).
# - Writes thorough manifest with debug_checks for reproducibility and audit.
...
# ===== Paths (no modifications to files) =====
...
# ===== Run flags (edit only flags; inputs untouched) =====
RUN_NAME                 = "RunFix_non_destructive_ctx128"
PRETRAIN_ENABLED         = True     # toggle pretrain transfer ON/OFF
...
EPOCHS_PRE               = 30
EPOCHS_FT                = 60
AUX_MAE_ALPHA            = 0.3      # stronger pressure on mu; does not change data
LOGSIGMA_CLIP_MIN        = -3.0
LOGSIGMA_CLIP_MAX        = 1.0
INIT_MU_BIAS_TO_PERSIST  = True     # initialize final mu bias to train persistence residual
REDUCED_CAPACITY         = False    # optional quick debug mode
...
# ===== Pretraining autoencoder (unchanged) =====
...
# ===== Probabilistic forecaster with clipped sigma and combined loss =====
...
# BROKEN: def finetune_prob_forecaster_aux(model, X_seq, y_next, epochs=EPOCHS_FT, alpha=AUX_MAE_ALPHA):
...
# ===== Main (non-destructive) =====
# BROKEN: if __name__ == "__main__":
    # Load frames (no modification)
    ...
    # Arrays and scaling (train stats only)
    ...
    # Debug: print train mean/std for Close and compare to any on-disk canonical manifest if available
    print("train_mean_close, train_std_close:", float(train_mean[0,3]), float(train_std[0,3]))

    # Context and sequences (no change to raw arrays)
    ...
    # Sanity: check first 10 window->target date alignment (audit)
    debug_checks = {}
    date_checks = []
# BROKEN:     for i in range(min(10, len(X_test))):
        win_end_idx = i + CONTEXT_LEN - 1
        target_idx = i + CONTEXT_LEN
        date_checks.append({
            "i": i,
            "window_end_date": str(df_test.loc[win_end_idx, "Date"]),
            "target_date": str(df_test.loc[target_idx, "Date"])
        })
    debug_checks["date_checks_head"] = date_checks

    # Quick sample: raw original Close windows vs targets (first 10) - for audit only
    sample_checks = []
# BROKEN:     for i in range(min(10, len(X_test))):
        sample_checks.append({
            "i": i,
            "window_last_close_unscaled": float(arr_test[i + CONTEXT_LEN - 1, 3]),
            "target_close_unscaled": float(arr_test[i + CONTEXT_LEN, 3]),
            "persistence_unscaled": float(arr_test[i + CONTEXT_LEN - 1, 3])
        })
    debug_checks["sample_checks_head"] = sample_checks

    # Optional: compute empirical persistence residual on train (scaled)
    train_pers = X_train[:, -1, 3:4]
    train_resid = y_train - train_pers
    mean_train_resid = float(np.mean(train_resid))  # scaled units

    # Pretraining (optional transfer only; does not change inputs)
    pre_hist = None
# BROKEN:     if PRETRAIN_ENABLED:
    ...
    # Build forecaster and optionally init mu bias to persistence residual
    forecaster = build_prob_forecaster_clipped(CONTEXT_LEN)

# BROKEN:     if INIT_MU_BIAS_TO_PERSIST:
        # Find mu Dense layer and initialise bias to mean_train_resid to start near persistence
# BROKEN:         try:
            mu_layer = forecaster.get_layer("mu")
            b_shape = mu_layer.bias.shape
            init_bias = np.full(b_shape, mean_train_resid, dtype=np.float32)
            mu_layer.bias.assign(init_bias)
            debug_checks["init_mu_bias_value"] = float(mean_train_resid)
# BROKEN:         except Exception as e:
            debug_checks["init_mu_bias_error"] = str(e)

    # Fine-tune (train+val merge for FT; test held out)
    X_ft = np.concatenate([X_train, X_val], axis=0)
    y_ft = np.concatenate([y_train, y_val], axis=0)
    ft_hist = finetune_prob_forecaster_aux(forecaster, X_ft, y_ft, epochs=EPOCHS_FT, alpha=AUX_MAE_ALPHA)

    # Predict on test (scaled)
    preds = forecaster.predict(X_test, verbose=0)
    mu = preds[:, 0:1]
    sigma = np.exp(preds[:, 1:2])  # log_sigma was clipped before exp
...
# BROKEN:     for i in range(min(20, len(mu))):
        diag.append({
            "i": i,
            "mu_scaled": float(mu[i]),
            "sigma_scaled": float(sigma[i]),
            "y_scaled": float(y_test[i]),
            "pers_scaled": float(pers[i]),
            "mu_unscaled": float(unscale(mu[i])),
            "y_unscaled": float(unscale(y_test[i])),
            "pers_unscaled": float(unscale(pers[i]))
        })
    debug_checks["diagnostic_head"] = diag
    debug_checks["mu_bias_scaled"] = float(np.mean(mu - y_test))
    debug_checks["mu_rmse_scaled"] = float(np.sqrt(np.mean((mu - y_test)**2)))
    debug_checks["sigma_mean_scaled"] = float(np.mean(sigma))
    debug_checks["sigma_std_scaled"] = float(np.std(sigma))
    ...
    manifest = {
        ...
        "flags": {
            ...
            "init_mu_bias_to_persist": INIT_MU_BIAS_TO_PERSIST
        },
        "files": {
