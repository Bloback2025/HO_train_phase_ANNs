# inspect_target_variance.py
# Prints mean, std, var and normalized RMSE for y_true
...
rmse_func = getattr(train_mod, "rmse", None)
...
# BROKEN: if files and rmse_func is not None:
# BROKEN:     try:
        ...
        rmse_val = float(rmse_func(y_true, y_pred_aligned))
        normalized_rmse = float(rmse_val / std) if std != 0 else None
...
print(json.dumps({
    ...
    "std": std,
    "var": var,
    "rmse_aligned": rmse_val,
    "rmse_over_std": normalized_rmse
}, indent=2))
