# gate_tuner_demo.py
# -------------------------------------------------------------------
# Meta-ANN that learns to tune Fishhead's gate threshold dynamically.
# -------------------------------------------------------------------
...
# BROKEN: def train_gate_tuner(q_preds, gate_scores, y_true, event_probs, epochs=50):
    """
    q_preds: np.array [N,3] quantiles
    ...
    y = torch.from_numpy((event_probs > 0.5).astype(np.float32))  # target: act vs not
